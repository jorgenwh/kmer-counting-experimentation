...
__global__ void init_hashtable_kernel(Table table, 
    const uint64_t *keys, const uint32_t size, const uint32_t capacity, const uint32_t cg_size) {
  int thread_id = (blockIdx.x * blockDim.x + threadIdx.x) % cg_size;
...
void init_hashtable(Table table, const uint64_t *keys, 
    const uint32_t size, const uint32_t capacity, const uint32_t cg_size) {
  int min_grid_size;
  ...
  int num_threads = size * cg_size;
  int grid_size = num_threads / thread_block_size + (num_threads % thread_block_size > 0);
  //int grid_size = (num_threads + thread_block_size - 1) / thread_block_size;
  init_hashtable_kernel<<<grid_size, thread_block_size>>>(table, keys, size, capacity, cg_size);
