...
CuHashTable::CuHashTable(
    const uint64_t *keys, const bool cuda_keys, const uint32_t size, const uint32_t capacity) {
  initialize(keys, cuda_keys, size, capacity);
}

void CuHashTable::initialize(
    const uint64_t *keys, const bool cuda_keys, const uint32_t size, const uint32_t capacity) {
  capacity_m = capacity;
  size_m = size;
...
void CuHashTable::get(const uint64_t *keys, uint32_t *counts, uint32_t size) const {
  uint64_t *d_keys;
  cuda_errchk(cudaMalloc(&d_keys, sizeof(uint64_t)*size));
  cuda_errchk(cudaMemcpy(d_keys, keys, sizeof(uint64_t)*size, cudaMemcpyHostToDevice));
...
void CuHashTable::getcu(const uint64_t *keys, uint32_t *counts, uint32_t size) const {
